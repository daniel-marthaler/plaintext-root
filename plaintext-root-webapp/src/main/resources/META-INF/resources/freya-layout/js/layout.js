...
PrimeFaces.widget.Freya = PrimeFaces.widget.BaseWidget.extend({
    ...
    _init: function() {
        ...
        // Restore menu state immediately after DOM elements are available
    ...
    restoreMenuState: function() {
        ...
        if (!this.isSlim() && !this.isHorizontal() && menuCookie) {
            ...
            for (var i = 0; i < this.expandedMenuitems.length; i++) {
                ...
                if (id) {
                    try {
                        // Escape special characters for jQuery selector: : and |
                        var escapedId = id.replace(/:/g, "\\:").replace(/\|/g, "\\|");
                        var menuitem = $("#" + escapedId);
                        ...
                        // Ignore jQuery selector errors for invalid IDs
                        // This can happen with dynamic menu IDs containing special characters
                    }
                }
            }
        }

        // Only restore layout-static if we're NOT in slim or horizontal mode
        // This prevents flickering when the menu mode doesn't support static sidebar
        var sidebarCookie = $.cookie('freya_menu_static');
        if(sidebarCookie && !this.isSlim() && !this.isHorizontal()) {
            this.wrapper.addClass('layout-static');
        }

        // Mark menu as ready - this makes submenus visible (prevents flickering)
        $(document.body).addClass('freya-menu-ready');
    },
