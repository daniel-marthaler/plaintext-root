...
class AdminEntityBackingBeanTest {
    ...
    @InjectMocks
    private AdminEntityBackingBean bean;
    ...
    @Test
    void loadAvailableEntities_loadsFromRegistry() {
        List<EntityDescriptor> entities = List.of(descriptor);
        when(registryService.getMandatAwareEntities()).thenReturn(entities);

        bean.loadAvailableEntities();

        assertEquals(entities, bean.getAvailableEntities());
        verify(registryService).getMandatAwareEntities();
    }
    ...
    @Test
    void loadEntities_loadsEntitiesForSelectedType() {
        ...
        doReturn(expected).when(entityService).findByMandat("TestEntity", "mandat1");
    ...
    void loadEntities_handlesException() {
        bean.setSelectedEntityType(descriptor);
        when(plaintextSecurity.getMandat()).thenReturn("mandat1");
        when(entityService.findByMandat("TestEntity", "mandat1")).thenThrow(new RuntimeException("DB error"));
    ...
    void saveEntity_savesEntityWithFieldValues() {
        ...
        doReturn(new ArrayList<>()).when(entityService).findByMandat(eq("TestEntity"), any());
    ...
    @Test
    void initThis_loadsEntitiesAndMandate() {
        List<EntityDescriptor> entities = List.of(descriptor);
        when(registryService.getMandatAwareEntities()).thenReturn(entities);
    ...
    void defaultState_hasEmptyCollections() {
        AdminEntityBackingBean fresh = new AdminEntityBackingBean();
